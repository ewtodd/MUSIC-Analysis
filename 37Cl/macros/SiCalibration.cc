...
void SiCalibration() {
  ...
  Int_t run_number;
  Int_t entrance_window; // 0 = NO, 1 = YES (0.9 mg/cm²)
  Int_t exit_window;     // 0 = NO, 1 = YES (1.3 mg/cm²)
  ...
  results->Branch("EntranceWindow", &entrance_window, "EntranceWindow/I");
  results->Branch("ExitWindow", &exit_window, "ExitWindow/I");
  ...
  std::map<int, int> entrance_config, exit_config;

  // Runs 21-22: No windows
  entrance_config[21] = 0;
  exit_config[21] = 0;
  entrance_config[22] = 0;
  exit_config[22] = 0;

  // Runs 23-24: Entrance window only
  entrance_config[23] = 1;
  exit_config[23] = 0;
  entrance_config[24] = 1;
  exit_config[24] = 0;

  // Runs 25-36: Both windows
  for (int run = 25; run <= 36; run++) {
    entrance_config[run] = 1;
    exit_config[run] = 1;
  }

  // Add pressure = 0 for runs 21-25
  for (int run = 21; run <= 25; run++) {
    gas_pressure[run] = 0;
  }
  ...
  for (int run = 21; run <= 36; run++) {
    ...
    run_number = run;
    entrance_window = entrance_config[run];
    exit_window = exit_config[run];
